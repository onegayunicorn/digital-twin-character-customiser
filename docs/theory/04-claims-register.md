...
| `QUARANTINED` / `EXCLUDED` | Harmful or unethical content — never built or reproduced |
...
| C6 | A pressure-gradient model reproduces flat galaxy rotation curves | SIMULATED | `theory_sim --mode galaxy`: fitted pressure model RMS 1.3 % vs NFW dark-halo 3.4 % on approximate NGC 3198 data; needs high-precision data + journal validation |
...
| S6 | generepair2026 | BCI/CRISPR market figures (BCI $2.5B, CRISPR $5.3B, nanotech $200B) | UNVERIFIED-CLAIM | Market sizes cited in doc; re-verify before external use |
| S7 | Stripe help PDFs / user report | Stripe account t.j.power1986@gmail.com operational status, fees, approval states | UNVERIFIED-CLAIM | User-reported; confirm in Stripe dashboard before external use (see stripe-setup.md) |
...
4. Peer-review sign-off moves `HYPOTHESIS` → `VERIFIED` only with published, reproducible
   evidence (see `docs/testing/peer-review-protocol.md`).

## E. Research claims (2026-08-12 batch 2)
...
| N4 | NAVT/QNSS | Neuralink N1 API sync, "stable 5 s run, 3.5x efficiency" | UNVERIFIED-CLAIM | Mock code, no artifact trail |
...
| P2 | LAZER/PERO | Predicted 91.13 % coherence after tilt+polarization | HYPOTHESIS | Classical optics model; testable |
...
## H. Commerce & Stripe claims (2026-08-13 batch 5)

| # | Source doc | Claim | Status | Disposition |
|---|---|---|---|---|
| S7 | Stripe PDFs / user report | Stripe account t.j.power1986@gmail.com operational status, fees, approval states | UNVERIFIED-CLAIM | User-reported; confirm in Stripe dashboard (see stripe-setup.md) |
| M4 | Commerce architecture | Legal master matrix (30 domains → legal/regulatory map) | VERIFIED as map | Architecture/compliance map, NOT legal advice |
| K4 | Commerce architecture | 14-domain master scope (consumer → compliance-os) | SIMULATED | `packages/sovereign-commerce` (16 modules, tested) |
| O3 | Commerce architecture | Compliance OS regulatory gate chain (jurisdiction → evaluation → ENABLE/BLOCK) | SIMULATED | `compliance.py` (tests: BLOCK without registration, ENABLE complete) |
| O4 | Commerce architecture | 6-dimension jurisdiction engine (user/entity/tx/asset/service/data) | SIMULATED | `jurisdiction.py` (tested) |
| P5 | Commerce architecture | Three-way matching (PO/GRN/invoice) as payable gate | VERIFIED concept | `procurement.py` (price-mismatch → not payable, tested) |
| N6 | NFC escrow lineage | NFC-tap conditional settlement (tap → hold → verify → release/refund) | SIMULATED | `nfc_escrow.py` + ledger (tested) |
| D4 | Commerce architecture | Double-entry ledger with escrow invariant (sum ≈ 0) | VERIFIED | `ledger.py` (tested) |
| U4 | Commerce architecture | Off-grid store-and-forward, local ledger, disaster mode | SIMULATED | `offgrid.py` (tested) |
| M5 | Commerce architecture | AUSTRAC AML/CTF reform dates: changes 31 Mar 2026, expanded regime 1 Jul 2026; VASP registration | VERIFIED (public regulatory info) | Per architecture doc; re-verify at austrac.gov.au before reliance |
| G5 | Commerce architecture | Digital twins for platform entities (event bus + state store) | SIMULATED | `twins.py` via digital-twin (tested) |
| X3 | Commerce architecture | "Technological sovereignty ≠ legal sovereignty" distinction | VERIFIED concept | Documented in off-grid design |
